...
export const getSettingsContainer = async (guild: Guild): Promise<Result<ContainerBuilder>> => {
  ...
  if (isUnsetSettings(settings)) currentSettings = "_No settings have been set yet_"
  else {
    ...
    const appsChannelResult = await attempt(async () => {
      if (!settings.appsChannelId) return
      const channel = await guild.channels.fetch(settings.appsChannelId)
    ...
    const appsCategoryResult = await attempt(async () => {
      if (!settings.appsCategoryId) return
      const category = await guild.channels.fetch(settings.appsCategoryId)
    ...
    const postLogsChannelResult = await attempt(async () => {
      if (!settings.postLogsChannelId) return
      const channel = await guild.channels.fetch(settings.postLogsChannelId)
      return channel?.toString()
    })
...
export const settingsCommand: Command = {
  ...
  run: async (interaction) => {
    ...
    const onCollect = async (i: ButtonInteraction) => {
    ...
    collector?.on("collect", (i) => void onCollect(i))
    collector?.on("end", () => void interaction.deleteReply())
  },
}
